...
- [ ] Class raster tiles and their pyramid are built from the stored rings at the nuclei store
      resolution (0.25 µm/px, plan §4.3), paletted rather than one plane per class.
- [ ] Tiles are served through the authenticated tile proxy, like tissue and biomarker tiles.
- [ ] Toggling the nuclei row's eye shows and hides the mask; it is aligned with the H&E at low and
      high zoom (check a vessel or a tissue edge, not just the centre).
- [ ] The nuclei layer has a defined place in the layer stack and composes with a tissue map and a
      phenotype layer switched on at the same time.
- [ ] Opacity is adjustable from the Nuclei panel; the class list and palette come from the
      artifact's own meta or the service catalog, never hardcoded in the frontend.
- [ ] The rasterised nucleus count for a region matches the stored vector count — one number, one
      source (decision D3).
