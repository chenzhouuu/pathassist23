...
export default function RightPanel() {
  ...
  const allTabs = [
    ...
    { id:'annotations', label:'Annot.', show: hasRole('annotation-users'), icon:
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>
      </svg> },
    { id:'panels',      label:'Panels', show: hasRole('ai-users'), badge: panels.length || null, icon:
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="app-sidepanel app-sidepanel-right" style={{ width:'var(--right-w)' }}>
      <div className="tab-bar shrink-0" style={{ display:'flex', borderBottom:'1px solid var(--border)' }}>
        {tabs.map(t => (
          <button key={t.id}
            title={t.label}
            className={`tab flex items-center gap-1 ${rightPanelTab === t.id ? 'active' : ''}`}
            onClick={() => setRightPanelTab(t.id)}
            style={{ flex:1, justifyContent:'center', padding:'6px 4px', fontSize:10, minWidth:0 }}>
            {t.icon}
            <span style={{ fontSize:9, letterSpacing:'0.01em' }}>{t.label}</span>
            {t.badge ? (
