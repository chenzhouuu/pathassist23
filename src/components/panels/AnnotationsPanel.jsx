...
function ElementRow({ el, idx, annColor }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 text-xs"
      ...
      <span className="capitalize w-16 shrink-0" style={{ color: 'var(--muted-hex)' }}>{typeLabel}</span>
      <span className="font-mono text-xs flex-1 truncate" style={{ color: 'var(--muted-hex)' }}>{coords}</span>
      {el.label?.value && (
...
function EditModal({ ann, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center"
      ...
      <div className="rounded-xl w-80 mx-4" style={{ background:'var(--bg-panel)', border:'1px solid var(--border-hex)' }}>
        {/* Header */}
        ...
        {loading ? (
          ...
          <div className="p-4 space-y-3">
            {/* Name */}
            <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--muted-hex)' }}>Name</label>
            ...
            {/* Description */}
            <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--muted-hex)' }}>Description</label>
            ...
            {/* Color */}
            <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--muted-hex)' }}>Color</label>
            ...
            {/* Line width */}
            <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--muted-hex)' }}>
            ...
            {loadingEl ? (
              ...
              <>
                ...
                <div className="px-3 py-1.5 text-xs font-mono"
                  style={{ color: 'var(--muted-hex)', borderTop:'1px solid var(--border-hex)' }}>
...
function DrawingControls() {
  ...
  return (
    <div className="px-2 py-2 space-y-2" style={{ borderBottom:'1px solid var(--border-hex)' }}>
      {/* Tool buttons */}
      ...
      {showSettings && (
        <div className="space-y-2 pb-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs w-10 shrink-0" style={{ color: 'var(--muted-hex)' }}>Color</span>
            <div className="flex items-center gap-1 flex-wrap">
              {DRAW_COLORS.map(c => (
          ...
          {/* Line width */}
          <div className="flex items-center gap-2">
            <span className="text-xs w-10 shrink-0" style={{ color: 'var(--muted-hex)' }}>Width</span>
            <input
              type="range" min={1} max={12} step={0.5}
            ...
            />
            <span className="text-xs font-mono w-6 text-right" style={{ color: 'var(--muted-hex)' }}>{drawLineWidth || 2}</span>
          </div>

          {/* Label */}
          <div className="flex items-center gap-2">
            <span className="text-xs w-10 shrink-0" style={{ color: 'var(--muted-hex)' }}>Label</span>
          ...
          {/* Group */}
          <div className="flex items-center gap-2">
            <span className="text-xs w-10 shrink-0" style={{ color: 'var(--muted-hex)' }}>Group</span>
      ...
      {drawingMode && (
        <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs"
          ...
          <button onClick={() => setDrawingMode(null)} style={{ color: 'var(--muted-hex)' }} title="Cancel (Esc)">✕</button>
        </div>
      )}
    </div>
  );
...
export default function AnnotationsPanel() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* List header */}
      <div className="flex items-center px-3 py-1.5 shrink-0" style={{ borderBottom:'1px solid var(--border-hex)' }}>
        ...
        <span className="ml-1.5 text-xs font-mono" style={{ color: 'var(--muted-hex)' }}>({annotations.length})</span>
