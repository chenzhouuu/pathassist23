...
A whole-slide marker map is hours of work holding the only worker thread, so "stop" has to be real
...
The queue and this file are both copies of the tissue service's (Inc 6 · 06). They stay copies
until 09 collapses the four `jobs.py` into one — and the contract is copied along with the code
precisely so that collapse has two spelled-out examples to be checked against.
"""
...
from biomarker_service.jobs import JobQueue
