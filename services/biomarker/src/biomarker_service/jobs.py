"""A single-consumer in-process job queue — the same shape preprocess uses.
...
gateway mirrors it into the durable ``preprocess_artifacts`` row (D2).
"""
...
def _release_cuda_cache() -> None:
    """Give the GPU back at the *job* boundary (mirrors preprocess.gpu).

    Deliberately at the boundary rather than inside the stage: a stage's own ``finally`` runs while
    an exception is still unwinding, and the live traceback keeps every frame — including the one
    holding the model — alive, so ``empty_cache()`` frees nothing.
    """
    try:
        import torch
