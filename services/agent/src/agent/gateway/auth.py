...
async def require_user(girder_token: str = Header(..., alias="Girder-Token")) -> dict:
    ...
    Ported from services/pathagent/gateway/auth.py: the copilot never trusts a
    client-supplied identity — it resolves the actor from the Girder session the
    viewer already holds. This is the "auth-on-read" seam; every future endpoint
    (plan, run, artifact fetch) depends on it.
    """
