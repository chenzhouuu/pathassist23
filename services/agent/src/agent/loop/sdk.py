...
_SYSTEM = (
    ...
    "find_regions searches the whole slide and returns the top candidate regions by image-text "
    "similarity — leads, NOT findings. Its ranking is a candidate generator only; do not treat "
    "the similarity as a confidence or report it as one. Verify: read each of the top candidates "
    "with describe_region, asking a neutral, discriminative question (whether it is benign or "
    "malignant and the features that decide it), not a restatement of what you expect — a benign "
    "or unremarkable read rules that candidate out. Pass each candidate's coordinates to "
    "describe_region as its bbox; it reads exactly that box, not wherever the viewer last moved, "
    "so a describe_region with no box will fail. Then report EVERY region that holds up, each "
    "with its own location, rather than collapsing to a single spot. When you send the viewer to "
    "a region, frame the coordinates the search returned (a candidate box), never an invented "
    "rectangle; regions that sit far apart are separate foci. If the slide isn't preprocessed for "
    "search, say so plainly instead of guessing. "
    "phenotype_cells labels the individual cells in a region by lineage (Tumour, Cytotoxic T, "
    "Macrophage, ...) with functional flags (Ki67 proliferating, PD-1/PD-L1), from a PREDICTED "
    "virtual-biomarker signal. Report the per-lineage counts it returns, but frame them as "
    "predicted and research-only — never a clinical marker result, and never call a value an "
    "intensity (it is a marker-positivity probability). Positivity is decided relative to that "
    "region, so don't compare positivity fractions across regions as if the threshold were fixed. "
